...
from modules import threat_intel, report_generator, deep_osint
from modules import data_aggregator
...
def main():
    ...
    parser.add_argument("-u", "--username", help="Пошук за нікнеймом (80+ платформ)")
    ...
    parser.add_argument("--deep", help="Глибокий OSINT-пошук (email, номер, IP, нік, домен)")
    parser.add_argument("--aggregate", help="Зберегти результати пошуку в базу")
    parser.add_argument("--connections", help="Знайти зв'язки для запиту")
    parser.add_argument("--analyze", action="store_true", help="Показати аналітику накопичених даних")
    ...
    # Якщо немає аргументів — показати help
    if len(sys.argv) < 2:
        parser.print_help()
        sys.exit(1)
    ...
    if args.deep:
        total_tasks += 1
        print(f"\n🧠 [10/{total_tasks}] Глибокий OSINT-пошук: {args.deep}")
        animate_loading()
        results["deep_osint"] = deep_osint.deep_search(args.deep)
        completed += 1
        progress_bar(completed, total_tasks, "Deep OSINT")
    
    if args.aggregate:
        total_tasks += 1
        print(f"\n💾 [11/{total_tasks}] Збереження даних для: {args.aggregate}")
        animate_loading()
        # Виконуємо пошук і зберігаємо
        search_result = deep_osint.deep_search(args.aggregate)
        data_aggregator.save_search(args.aggregate, "auto", search_result)
        completed += 1
        progress_bar(completed, total_tasks, "Data Aggregation")
        results["aggregated"] = f"Дані для {args.aggregate} збережено"
    
    if args.connections:
        total_tasks += 1
        print(f"\n🔗 [12/{total_tasks}] Пошук зв'язків для: {args.connections}")
        animate_loading()
        conns = data_aggregator.find_connections(args.connections)
        completed += 1
        progress_bar(completed, total_tasks, "Connections Search")
        results["connections"] = conns
    
    if args.analyze:
        total_tasks += 1
        print(f"\n📊 [13/{total_tasks}] Аналіз даних")
        animate_loading()
        report = data_aggregator.generate_analysis_report()
        completed += 1
        progress_bar(completed, total_tasks, "Data Analysis")
    ...
    print_status("✅ Роботу завершено", "success")
...
def run_demo():
    ...
    demo_results = {
        ...
        "deep_osint": {
            ...
            "data": {
                "username": {"found_on": 8},
                "graph": {"connections": ["neo", "saturn"]},
                "social": {"twitter": "Профіль існує"}
            }
        },
        "analysis_report": {
            "total_queries": 42,
            "connections": 7,
            "type_distribution": {"username": 20, "email": 15, "ip": 7}
        }
    }
